...
from sbox.utils import get_object_from_queryset_or_404
from utilities.error_views import render_forbidden_response
...
def use_ad_hoc_script(request, membership_id, script_id):
    ...
    return start_data_entry(request,
                            first_ob.token,
                            reverse('participant_overview',
                                    args=(membership.user.id, )),
                            "Ad hoc script")
...
def preview_reply(request, id):
    ...
    scoresheets = ScoreSheet.objects.all()

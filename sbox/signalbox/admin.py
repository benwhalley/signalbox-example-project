...
class StudyAdmin(admin.ModelAdmin):
    ...
    list_display = ['slug', 'visible', 'paused']
    ...
    fieldsets = (
        ("Main details", {
            ...
            'fields': (
                'name', 'slug', 'study_email', 'administrators',
                'visible', 'paused', 'show_study_condition_to_user'
            )
        }),
        ...
        ('SMS and Telephone settings', {
            'classes': ('collapse',),
            'fields': (
                'outbound_telephone_number', 'outbound_sms_number',
                'inbound_number',
...
class ObservationAdmin(VersionAdmin):
    ...
    list_display = ["label", "token", "user", "due", "status", "script_type", "n_questions", "n_questions_incomplete"]
    list_filter = ('due_original', 'status', 'attempt_count', 'dyad__study', 'dyad__user__userprofile__site', 'created_by_script__script_type', )
    ...
    def resend_obs(self, request, queryset):
        ...
        messages.add_message(request, messages.INFO,
            "Attempting to resend %s observations. ID#: %s" % (len(obs), ", ".join([str(i[1].id) for i in obs])))
...
class ScriptAdmin(admin.ModelAdmin):
    ...
    list_display = ['admin_display_name', 'script_type', 'preview_of_datetimes']
...
class AnswerAdmin(VersionAdmin):
    ...
    readonly_fields = ['question', 'other_variable_name', 'reply',
                       'answer', 'choices', 'meta']
    search_fields = ['reply__token', 'reply__observation__dyad__user__username',
...
class ReplyAdmin(VersionAdmin):

    date_hierarchy = 'last_submit'
    save_on_top = True
    readonly_fields = ['complete', 'observation', 'user', 'asker',
                       'current_page', 'last_submit', 'started']
    list_display = ['token', 'entry_method', 'user', 'complete',
                    'last_submit', 'observation', ]
    list_filter = ['observation__dyad__study', 'entry_method']
    ...
    fieldsets = (
        ...
        ("Status", {
            'fields': ('complete', 'last_submit', 'current_page')
        }),

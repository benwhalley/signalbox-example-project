...
ALL_MENU_ITEMS = {
    ...
    'answerphone': (('Check new answerphone messages'),
                    reverse(
                    'admin:twiliobox_answerphonemessage_changelist') + "?category__isnull=True"),
    'todo': (('Show observations currently due to be sent'), reverse('show_todo')),
    'export': (('Export data'), reverse('export_data')),
    'history': (('View revision history'), reverse('check_history')),
...
PER_GROUP_MENUS = {
    ...
    'Research Assistants': [ALL_MENU_ITEMS[k] for k in
                            ['add_participant', 'add_staff', 'followups', 'todo', 'answerphone', 'observations_outstanding']],

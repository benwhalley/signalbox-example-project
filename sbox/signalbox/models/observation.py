...
from utilities.sanitise_redirects import sanitise_user_supplied_redirect
...
class ReminderInstance(models.Model):
    ...
    def _send_sms_reminder(self):
        if not client:
            return (False, "Twilio settings not available")

        to_number = self.observation.user.get_profile().mobile()
        from_number = self.observation.dyad.study.get_outbound_sms_number()
...
class Observation(models.Model):
    ...
    label = models.CharField(blank=True, null=True, max_length=1024)
    ...
    def completion_data(self):
        ...
        answered_questions = set([(i.question, i.other_variable_name)
                                    for i in answered])
    ...
    def ready_to_send(self):
        ...
        # todo... reinstate this, but only for phone calls
    ...
    def make_reply(self, request, external_id=None, success_url=None, entry_method=None):
        ...
        if not self.can_add_answers():
            raise Exception("No answers can be added for this type of observation.")
        ...
        if success_url:
            goto = sanitise_user_supplied_redirect(request, success_url)
        else:
            if  self.created_by_script and self.created_by_script.redirect_url:
                goto = self.created_by_script.redirect_url

            if entry_method == "double_entry":
                goto = reverse('admin:signalbox_membership_change', args=(self.dyad.id,))

        reply.redirect_to = goto
        reply.save()

        if reply.entry_method is "double_entry":

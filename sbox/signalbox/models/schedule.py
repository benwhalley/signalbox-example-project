...
safehelp = lambda x: mark_safe(markdown.markdown(x))




NATURAL_DATE_SYNTAX_HELP = safehelp(u"""
...
class ScriptType(models.Model):
    ...
    observation_subclass_name = models.CharField(blank=True, max_length=80)
...
will not result in an error, but won't produce any output either. """))
    ...
    success_message = models.TextField(default="Questionnaire complete.",
        help_text="""Message which appears in banner on users screen on
    ...
    RRULES = [(i,i) for i in
        "YEARLY MONTHLY WEEKLY DAILY HOURLY MINUTELY SECONDLY".split(" ")]
    ...
    def preview_of_datetimes(self):
        ...
        return safehelp("<br>".join(map(code, list(self.datetimes()))))
    ...
    def make_observations(self, membership):
        ...
        def _create_label(labelstring, context=None):
            context = context or {}
            return Template(labelstring).render(Context(context))
        ...
        for time, index in times_indexes:
            observations.append(Observation(due_original=time,
                n_in_sequence = index,
                label = _create_label(self.label, {'i': index, 'n': ordinal(index),
                                                    'script': self,

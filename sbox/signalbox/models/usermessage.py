...
class UserMessage(models.Model):
    ...
    message_to = models.ForeignKey(
        User, related_name="to", verbose_name="To", blank=False)
    message_from = models.ForeignKey(
        User, blank=False, null=False, related_name="from")
    ...
    def _send_sms(self):
        if not client:
            raise Exception("Twilio settings not available when attempting to send an SMS message")

        to_number = international_string(self.message_to.get_profile().mobile)
        from_number = settings.SMS_CALLERID

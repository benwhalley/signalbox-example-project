...
from signalbox.models.validators import is_24_hour, is_twilio_approved, only_includes_allowed_fields
...
class Study(models.Model):
    ...
    visible = models.BooleanField(default=False, help_text="""Controls the
        display of the study on the /studies/ page.""", db_index=True)
    ...
    outbound_telephone_number = models.CharField(blank=True, max_length=100,
        help_text="""Used as the CallerID for automated calls.""",
        validators=[is_twilio_approved])

    outbound_sms_number = models.CharField(blank=True, max_length=100,
        help_text="""Used as the CallerID for SMS messages""",
        validators=[is_twilio_approved])

    inbound_number = models.CharField(blank=True, max_length=100,
        help_text="""Twilio number used to accept calls and SMS messages.""",
        validators=[is_twilio_approved])

    ad_hoc_scripts = models.ManyToManyField('signalbox.Script', blank=True, null=True,
    ...
    createifs = models.ManyToManyField('signalbox.ObservationCreator', null=True, blank=True,
    ...
    def get_outbound_telephone_number(self):
        """Return a number to use as the callerid when making calls via Twilio."""

        return self.outbound_telephone_number or settings.CALLERID

    def get_outbound_sms_number(self):
        """Return a telephone number to use as the return address for SMS messages."""

        return self.outbound_sms_number or settings.SMS_CALLERID
    ...
    allocation_method = models.CharField(max_length=85, default=settings.ALLOCATION_CHOICES[1][0],
    ...
    randomisation_probability = models.DecimalField(blank=True, null=True, max_digits=2,
    ...
    scoresheets = models.ManyToManyField('signalbox.ScoreSheet', blank=True, null=True,
        ...
        this study. For example, if you would like to automatically calculate
        the total score on a screening instrument and make this visible within
        the admin, then first create a ScoreSheet object which defines the
        variables to be summed, then attach it to the study here.""",
        verbose_name="""Create these summary scores for Replies""")
    ...
    def scripts_used_by_this_study(self):
        return set(
            itertools.chain(*[i.scripts.all() for i in self.studycondition_set.all()])
        )
    ...
    def questions_used(self):
        """Return list of all :class:`Question`s used to record an :class:`Answer` for this class:`Study`."""
    ...
    def clean(self):
        if (self.allocation_method == "balanced_groups_adaptive_randomisation"
                and (not self.randomisation_probability)):
...
class StudyPeriod(models.Model):
    ...
    tag = models.SlugField(help_text="Used for the stata variable name. No spaces or dashes.")
    ...
    def _participant_ranges(self):
        ...
        starts = [i['date_randomised'] + timedelta(days=self.start) for i in everything]
        ends = [i['date_randomised'] + timedelta(days=self.end) for i in everything]
    ...
    def participants(self):
        ...
        inrange = [i for i in people if in_range(datetime.today().date(), i[1])]
    ...
    def observations(self):
        ...
        allobs = allobs.filter(due__lte=datetime.now())  # just those due before now
        allobs = allobs.values('id', 'due', 'dyad__date_randomised')

        # create ranges and ids and zip together
        starts = [i['dyad__date_randomised'] + timedelta(days=self.start) for i in allobs]
        ends = [i['dyad__date_randomised'] + timedelta(days=self.end) for i in allobs]
        ranges = zip(starts, ends)

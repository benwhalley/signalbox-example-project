...
def say_or_play(response, question, reply=None):
    ...
    if question.question.audio:
        response.play(question.question.audio.url)
    else:
        response.say(question.question.display_text(reply=reply), language=settings.TTS_LANGUAGE, voice=settings.TTS_VOICE)
